...
func (lc *litAfClient) Graph(textArgs []string) error {
	...
	err := lc.rpccon.Call(lc.peerId, "", "LitRPC.GetChannelMap", args, reply)
	if err != nil {
		return err
	}
...
func (lc *litAfClient) RequestAsync() {
	for {
		...
		err := lc.rpccon.Call(lc.peerId, "", "LitRPC.GetMessages", args, reply)
		if err != nil {
...
func (lc *litAfClient) Lis(textArgs []string) error {
	...
	err := lc.rpccon.Call(lc.peerId, "", "LitRPC.Listen", args, reply)
	if err != nil {
		return err
	}
...
func (lc *litAfClient) Connect(textArgs []string) error {
	...
	err := lc.rpccon.Call(lc.peerId, "", "LitRPC.Connect", args, reply)
	if err != nil {
		return err
	}
...
func (lc *litAfClient) Say(textArgs []string) error {
	...
	err = lc.rpccon.Call(lc.peerId, "", "LitRPC.Say", args, reply)
	if err != nil {
		return err
	}
	fmt.Fprintf(color.Output, "%s\n", reply.Status)
