...
func (lc *litAfClient) Send(textArgs []string) error {
	...
	*/
	...
	err = lc.rpccon.Call(lc.peerId, "", "LitRPC.Send", args, reply)
	if err != nil {
		return err
	}
...
func (lc *litAfClient) Sweep(textArgs []string) error {
	...
	err = lc.rpccon.Call(lc.peerId, "", "LitRPC.Sweep", args, reply)
	if err != nil {
		return err
	}
...
func (lc *litAfClient) Fan(textArgs []string) error {
	...
	err = lc.rpccon.Call(lc.peerId, "", "LitRPC.Fanout", args, reply)
	if err != nil {
		return err
	}
...
func (lc *litAfClient) Fee(textArgs []string) error {
	...
	if set {
		err := lc.rpccon.Call(lc.peerId, "", "LitRPC.SetFee", SetArgs, reply)
		if err != nil {
			return err
		}
	} else {
		err := lc.rpccon.Call(lc.peerId, "", "LitRPC.GetFee", GetArgs, reply)
		if err != nil {
			return err
		}
...
func (lc *litAfClient) Address(textArgs []string) error {
	...
	fmt.Printf("args: %v\n", args)
	err := lc.rpccon.Call(lc.peerId, "", "LitRPC.Address", args, reply)
	if err != nil {
		return err
	}
